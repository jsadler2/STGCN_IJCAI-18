...
def temporal_conv_layer(x, Kt, c_in, c_out, act_func='relu'):
    ...
    # This if statement is defining x_input which is used if the activation function is either GLU or ReLU
    if c_in > c_out:
    ...
    # [jeff] in this if statement it seems like they are getting weights (wt), and biases (bt) and creating a
    # convolution. The convolution is then adjusted by an activation function
    if act_func == 'GLU':
        # gated liner unit
        ...
        # [jeff] interesting here that they use a "2d" convolution. to me, since it's a temporal dataset, i would have
        # guessed that it would have been 1d
        x_conv = tf.nn.conv2d(x, wt, strides=[1, 1, 1, 1], padding='VALID') + bt
        return (x_conv[:, :, :, 0:c_out] + x_input) * tf.nn.sigmoid(x_conv[:, :, :, -c_out:])
    else:
        # [jeff] the only diff between the following line and for 'bt' and the lines above is that c_out is multiplied
        # by 2
